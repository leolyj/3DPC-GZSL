...
class SemanticKittiDataset(PointCloudDataset):
    ...
    def __init__(self, config, args, set='training', balance_classes=True, ignored_labels = np.sort([0, 3, 4, 7, 19]), visual_set=False):
        PointCloudDataset.__init__(self, 'SemanticKitti')

        ##########################
        # Parameters for the files
        ##########################
        self.unseen_idx_list = ignored_labels.tolist()
        # Dataset folder
        self.path =None
        home_path = str('../../')
        if visual_set:
            self.path =os.path.join(home_path,"data/semantic_kitti")
        else:
            try:
                import valeodata
                self.path = os.path.join(valeodata.download('semantic_kitti_bjoern'),"dataset")
            except Exception as e:
                print("Import valeodata is not available: {}".format(e))
                self.path =os.path.join(home_path,"data/semantic_kitti")
        self.counter_classes = np.zeros(20)
        self.counter_frames = 0

        self.args = args
        #Get the word representations
        self.Glove_num_dict, self.w2v_num_dict, self.img_embd_array = get_word_vector(dataset="sk")
        ...
        print("Ignored Labels in the dataset {}".format(self.ignored_labels))
        ##################
        # Other parameters
        ##################
        ...
        print("Self potenttials shape {}".format(self.potentials.shape))
    ...
    def __getitem__(self, batch_i):
        ...
        while True:
            ...
            with self.worker_lock:
                ...
                # Update epoch indice
                self.epoch_i += 1
                self.epoch_i = self.epoch_i % 4400

            s_ind, f_ind = self.all_inds[ind] #man bekommt sequence und frame index 
            ...
            unique_classes, count_unique_elements = np.unique(merged_labels, return_counts = True)
            if self.set=="training" and any(x in unique_classes.tolist() for x in self.unseen_idx_list):
                continue
            if self.set=="validation_split" and any(x in unique_classes.tolist() for x in [0, 3, 4, 7, 19]): 
                continue

            self.counter_classes[unique_classes] = self.counter_classes[unique_classes] + 1
        ...
        stack_lengths = np.array([pp.shape[0] for pp in p_list], dtype=np.int32) #Hier ist die laenge der einzelnen Objekte verbucht
        ...
        #######################
        #
        #   Points, radius_neighbors, pooling indices for each layers
        #

       
        if self.args.load_embedding == "w2v":     
            attributes = np.array(list(map(lambda x: self.w2v_num_dict[x],np.squeeze(labels.astype(np.int32)))), dtype=np.float32)
        
        elif self.args.load_embedding == "glove": 
            attributes = np.array(list(map(lambda x: self.Glove_num_dict[x],np.squeeze(labels.astype(np.int32)))), dtype=np.float32)
        
        elif self.args.load_embedding == "glove_w2v":
            w2v_gt_attributes = np.array(list(map(lambda x: self.w2v_num_dict[x],np.squeeze(labels.astype(np.int32)))), dtype=np.float32)
            glove_gt_attributes = np.array(list(map(lambda x: self.Glove_num_dict[x],np.squeeze(labels.astype(np.int32)))), dtype=np.float32)
            attributes = np.hstack((glove_gt_attributes, w2v_gt_attributes.astype(np.float32)))
        else: 
            attributes = np.array(list(map(lambda x: self.img_embd_array[x],np.squeeze(labels.astype(np.float32)))), dtype=np.float32)
        ...
        input_list += [scales, rots, frame_inds, frame_centers, r_inds_list, r_mask_list, val_labels_list, attributes]
...
class SemanticKittiSampler(Sampler):
    ...
    def __iter__(self):
        ...
        if self.dataset.balance_classes:
            ...
            #print("Self data potentials shape {}".format(self.dataset.potentials.shape))
            ...
            #print("Gen indices {}".format(gen_indices))
            #print("Gen indices shape {}".format(gen_indices.shape))
            # Update potentials (Change the order for the next epoch)
            self.dataset.potentials[gen_indices] = torch.ceil(self.dataset.potentials[gen_indices])
            self.dataset.potentials[gen_indices] += torch.from_numpy(np.random.rand(gen_indices.shape[0]) * 0.1 + 0.1)

            # Update epoch inds
            #print("Self dataset epoch inds {}".format(self.dataset.epoch_inds))
            #print("self datast epoch shape {}".format(self.dataset.epoch_inds.shape))
            self.dataset.epoch_inds += gen_indices
    ...
    def calib_max_in(self, config, dataloader, untouched_ratio=0.8, verbose=True, force_redo=False):
        """
        Method performing batch and radius_neighbors calibration.
            Batch calibration: Set "batch_limit" (the maximum number of points allowed in every batch) so that the
                               average batch size (number of stacked pointclouds) is the one asked.
        Neighbors calibration: Set the "neighborhood_limits" (the maximum number of radius_neighbors allowed in convolutions)
    ...
    def calibration(self, dataloader, untouched_ratio=0.9, verbose=False, force_redo=False):
        ...
        if verbose:
            print('Check radius_neighbors limit dictionary')
            for layer_ind in range(self.dataset.config.num_layers):
                dl = self.dataset.config.first_subsampling_dl * (2**layer_ind)
                if self.dataset.config.deform_layers[layer_ind]:
                    r = dl * self.dataset.config.deform_radius
        ...
        if redo:
            ...
            # From config parameter, compute higher bound of radius_neighbors number in a neighborhood
            ...
            # Use collected neighbor histogram to get radius_neighbors limit
            ...
            if verbose:
                ...
                print('\n**************************************************\n')
                print('\nchosen radius_neighbors limits: ', percentiles)
                print()
...
class SemanticKittiCustomBatch:
    ...
    def __init__(self, input_list):
        ...
        ind += 1
        self.attributes = torch.from_numpy(input_list[ind])
    ...
    def pin_memory(self):
        ...
        self.attributes = self.attributes.pin_memory()
    ...
    def unstack_neighbors(self, layer=None):
        """Unstack the radius_neighbors indices"""
        return self.unstack_elements('radius_neighbors', layer)
    ...
    def unstack_elements(self, element_name, layer=None, to_numpy=True):
        ...
        elif element_name == 'radius_neighbors':
        ...
        for layer_i, layer_elems in enumerate(elements):
            ...
            if layer is None or layer == layer_i:
                ...
                for b_i, length in enumerate(lengths):
                    ...
                    if element_name == 'radius_neighbors':
