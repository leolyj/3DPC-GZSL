...
from gzsl3d.kpconv.kernels.kernel_points import load_kernels
from gzsl3d.kpconv.utils.ply import write_ply
...
def closest_pool(x, inds):
    """
    Pools features from the closest radius_neighbors. WARNING: this function assumes the radius_neighbors are ordered.
...
class KPConv(nn.Module):
    ...
    def forward(self, q_pts, s_pts, neighb_inds, x):
        ...
        # Add a fake point in the last row for shadow radius_neighbors
        ...
        if self.deformable:
            ...
            # Boolean of the radius_neighbors in range of a kernel point [n_points, n_neighbors]
            in_range = torch.any(sq_distances < self.KP_extent ** 2, dim=2).type(torch.int32)

            # New value of max radius_neighbors
            new_max_neighb = torch.max(torch.sum(in_range, dim=1))

            # For each row of radius_neighbors, indices of the ones that are in range [n_points, new_max_neighb]
            ...
            # New shadow radius_neighbors have to point to the last shadow point
        ...
        # Add a zero feature for shadow radius_neighbors
        x = torch.cat((x, torch.zeros_like(x[:1, :])), 0)
